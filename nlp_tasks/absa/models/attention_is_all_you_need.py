...
def attention_is_all_you_need_multi_label(max_seq_len, input_dim, output_dim, embedding_matrix, num_class):
    """rnn_attention_multi_label"""
    inp = Input(shape=(max_seq_len,))
    x = Embedding(input_dim, output_dim, weights=[embedding_matrix], trainable=False)(inp)
    x = Position_Embedding()(x)
    x = Attention(8, 40)([x, x, x])

    outputs = []
    for i in range(num_class):
        x_i = keras_layers.AttLayer(10, i)(x)
