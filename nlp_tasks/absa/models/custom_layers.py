...
class Attention(Layer):
 ...
 e: The layer has been tested with Keras 2.0.6
    ...
    def __init__(self, W_regularizer=None, u_regularizer=None, b_regularizer=None, W_constraint=None,
                 ...
                 return_attend_weight=True, u_size=None, just_average=False, **kwargs):
        ...
        self.u_size = u_size
        self.just_average = just_average
    ...
    def build(self, input_shape):
        ...
        u_size = self.u_size
        if u_size is None:
            u_size = input_shape[-1]
        if self.use_W:
            self.W = self.add_weight(shape=(input_shape[-1], u_size,),  initializer=self.init,
                                     name='{}_W'.format(self.name), regularizer=self.W_regularizer,
                                     constraint=self.W_constraint)
        if self.use_bias:
            self.b = self.add_weight(shape=(u_size,), initializer='zero', name='{}_b'.format(self.name),
                                     regularizer=self.b_regularizer, constraint=self.b_constraint)
        if self.just_average:
            self.u = self.add_weight(shape=(u_size,), initializer=initializers.Zeros(), name='{}_u'.format(self.name),
                                     regularizer=self.u_regularizer, constraint=self.u_constraint, trainable=False)
        else:
            self.u = self.add_weight(shape=(u_size,), initializer=self.init, name='{}_u'.format(self.name),
    ...
    def call(self, x, mask=None):
        if self.use_W:
            if self.use_bias:
                x = K.tanh(K.bias_add(K.dot(x, self.W), self.b))
            else:
...
class TopicAttention(Layer):
 ...
 e: The layer has been tested with Keras 2.0.6
    ...
    def __init__(self, W_regularizer=None, u_regularizer=None, b_regularizer=None, W_constraint=None,
                 u_constraint=None, b_constraint=None, use_W=True, use_bias=False, return_self_attend=False,
                 return_attend_weight=True, topic_num=1, **kwargs):
        self.supports_masking = True
    ...
    def build(self, input_shape):
        ...
        if self.use_W:
            ...
            for i in range(self.topic_num):
                self.W_matrix.append(self.add_weight(shape=(input_shape[-1], input_shape[-1],),
        ...
        if self.use_bias:
            self.b_matrix = []
            self.b_matrix.append(self.add_weight(shape=(input_shape[1],), initializer='zero', name='{}_b'.format(self.name),
                                     regularizer=self.b_regularizer, constraint=self.b_constraint))

        self.u_matrix = self.add_weight(shape=(self.topic_num, input_shape[-1]), initializer=self.init, name='{}_u'.format(self.name),
                                 regularizer=self.u_regularizer, constraint=self.u_constraint)

        super(TopicAttention, self).build(input_shape)
    ...
    def call(self, x, mask=None):
        ...
        for i in range(self.topic_num):
            if self.use_W:
                x = K.tanh(K.dot(x, self.W_matrix[i]))

            ait = Attention.dot_product(x, self.u_matrix[i])
            if self.use_bias:
                ait += self.b_matrix[i]
            ...
            if self.return_self_attend:
                attend_output = K.sum(x * K.expand_dims(a), axis=1)
                if self.return_attend_weight:
                    result.extend([attend_output, a])
                else:
                    result.append(attend_output)
            else:
                result.append(a)
        return result
    ...
    def compute_output_shape(self, input_shape):
        ...
        for i in range(self.topic_num):
            if self.return_self_attend:
                if self.return_attend_weight:
                    result.extend((input_shape[0], input_shape[-1]), (input_shape[0], input_shape[1]))
                else:
                    result.append((input_shape[0], input_shape[-1]))
            else:
                result.append((input_shape[0], input_shape[1]))
        return result
